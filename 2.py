...
def start_game():
    ...
    letal_b = ButtonGame(170, 80, 1, 100)  # влияет на вероятность смерти
    zaraz_b = ButtonGame(170, 80, 2, 100)  # влияет на вероятность заражения
    imun_b = ButtonGame(170, 80, 3, 100)  # влияет на время выздоровления
    mutation_b = ButtonGame(170, 80, 4, 1000)  # мутация

    # Кнопки взаимодействия с лабораторией
    lab_enter = ButtonGame(170, 80, 5, 0)# Вход в лабораторию
    lab_back = ButtonGame(170, 80, 6, 0)# Выход из лаборатории
    ...
    while not finished:
        ...
        if lab_enter.indicator:
            ...
            scale_letal.draw()
            letal_b.draw(50, 650, 'Lethality+1', 'Cost: ' + str(letal_b.cost), None, 30)
            zaraz_b.draw(400, 650, 'Infection+1', 'Cost: ' + str(zaraz_b.cost), None, 30)  # тоже кнопки
            imun_b.draw(750, 650, 'Immune+1', 'Cost: ' + str(imun_b.cost), None, 30)
            mutation_b.draw(1100, 400, 'Mutation', 'Cost: ' + str(mutation_b.cost), None, 30)
            lab_back.draw(1100, 650, 'Exit the', 'Lab', None, 30)
            if lab_back.indicator:
                lab_enter.indicator = False
        else:
        ...
        for i in range(len(countries)):
            for j in range(len(countries[i].Cities)):
                if zaraz_b.indicator and zaraz_b.cost <= score:
                    countries[i].Cities[j].propability += 0.04
                if letal_b.indicator and letal_b.cost <= score:
                    countries[i].Cities[j].Dpropability += 0.04
                if imun_b.indicator and imun_b.cost <= score:
        ...
        #Обновление возможности мутации
        if mutation_b.indicator and  mutation_b.cost <= score:
            score -= mutation_b.cost
            mutation_b.cost += 1000
            button_sound = pygame.mixer.Sound('button2.wav')
            pygame.mixer.Sound.play(button_sound)
            pygame.time.delay(300)

        #Обновление эволюций
        if (imun_b.indicator or zaraz_b.indicator or letal_b.indicator) and imun_b.cost <= score:
            score -= imun_b.cost
            imun_b.cost += 100
            letal_b.cost += 100
            zaraz_b.cost += 100
            if imun_b.indicator:
                imun_b.lvl += 1
            if zaraz_b.indicator:
                zaraz_b.lvl += 1
            if letal_b.indicator:
                letal_b.lvl += 1
            button_sound = pygame.mixer.Sound('button2.wav')
            pygame.mixer.Sound.play(button_sound)
            pygame.time.delay(300)
        ...
        mutation_b.indicator = False
        imun_b.indicator = False
        letal_b.indicator = False
        zaraz_b.indicator = False
...
class ButtonGame:
    ...
    def __init__(self, width, height, type, cost):
        self.width = width
        self.height = height
        self.type = type
    ...
    def draw(self, x, y, message1, message2, action=None, font_size=50):
        ...
        if x < mouse[0] < x + self.width and y < mouse[1] < y + self.height and type == 4:
            pygame.draw.rect(screen, self.active_color, (x, y, self.width, self.height))
